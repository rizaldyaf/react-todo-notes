...
class CheckComposer extends Component {
    state = {
        lastEdited:"",
        mode:"create",
        confirmDialog:false,
    ...
    handlePaste = (pastedText, index) => {
        let newChecks = [...this.state.checklists]; // Clone the current checklists state
        let split = pastedText.split("\n").map((el) => el.replace("\r", "")); // Split and clean pasted text
        let elementsToInsert = split.filter((text)=>text).map((text) => ({ checked: false, text })); // Convert lines into objects
        console.log("split", split)
        // Use splice to remove the element at index and insert new elements
        newChecks.splice(index, 1, ...elementsToInsert);
    
        console.log(newChecks); // Log the updated array
        this.setState({
            checklists: newChecks, // Update state with the modified array
        });
    };

    toggleCheck = (value, newIndex) => {
        this.setState({
            checklists:this.state.checklists.map((cl, index)=>{
                if (newIndex === index) {
                    return {...cl, checked:value}
                }
                return cl
            })
        })
    ...
    handleChangeText = (value, newIndex) => {
        this.setState({
            checklists:this.state.checklists.map((cl, index)=>{
                if (newIndex === index) {
                    return {...cl, text:value}
                }
                return cl
            })
        })
    ...
    render() {
        ...
        <div className={classes.container}>
            ...
            <div className="pageWrapper">
                ...
                <div className='checklists'>
                    ...
                    <div className='clItems'>
                        {this.state.checklists.map((check, index)=>{
                            return <div className='clItem' key={index}>
                                <CheckBox 
                                    value={check.checked}
                                    onChange={(value)=>this.toggleCheck(value, index)}   
                                />
                                <TextBox 
                                    disableMargin
                                    value={check.text}
                                    onPaste={(t)=>this.handlePaste(t, index)}
                                    onChange={(value)=>this.handleChangeText(value, index)}
                                />
                                <Button type="icon">
                                    <span className='icon'>
                                        &#xE74D;
                        ...
                        <div className='clItem'>
                            <Button fullWidth onClick={()=>this.handleAddCheck()}>
                                <span className="icon">
                                    &#xE710;
                                </span>
                                Add New Item
            ...
            >
                <div className='dialogTitle'>Delete Checklist</div>
                <div className='dialogContent'>Are you sure want to delete this checklist?</div>
            ...
            <dialog 
                ...
                onKeyDown={(e)=>this.handleDialogKeyDown(e)}
            >
                <div className='dialogTitle'>Lock Checklist</div>
                <div className='dialogContent'>
                    <p>Create a new password for this checklist</p>
            ...
            <dialog 
                ...
                onKeyDown={(e)=>this.handleDialogKeyDown(e)}
            >
                <div className='dialogTitle'>Unlock Checklist</div>
                <div className='dialogContent'>
                    <p>Unlock to remove password for this checklist</p>
                    <PasswordBox 
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(withStyles(styles)(CheckComposer)))